...
function loadDeepSeekConfig({ fatal = true } = {}) {
  ...
  for (const file of paths) {
    try {
      ...
      logger.error(`[DS-API] Could not load auth config ${file}: ${_e.message}`);
    }
  }
...
function selectAccountForSession(session) {
  ...
  if (ready.length === 0) {
    ...
    if (busy.length > 0) {
      throw new AppError('RATE_LIMIT', { message: `All DeepSeek auth accounts are at max concurrency (${config.MAX_CONCURRENT_PER_ACCOUNT}). Try again later.` });
    }
    ...
    if (waiting) {
      ...
      throw new AppError('CIRCUIT_OPEN', { message: `All DeepSeek auth accounts are cooling down. Retry in ~${waitSec}s or import a fresh account with npm run auth:import.`, retryAfter: waitSec });
    }
    throw new AppError('AUTH_EXPIRED', { message: 'No valid DeepSeek auth accounts. Run npm run auth or npm run auth:import.' });
  }
