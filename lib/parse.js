...
function formatToolDefinitions(tools) {
  ...
  let text = '\n\n<tool_instructions>\n';
  text += 'You are an AI assistant with tool-calling capability.\n';
  text += 'When you need to perform an action, you MUST respond with EXACTLY this format:\n';
  text += 'TOOL_CALL: <function_name>\narguments: <JSON>\n';
  text += 'Then wait for the tool result before continuing.\n\n';
  text += 'RULES:\n';
  text += '1. Output ONLY the tool call — no text before, no text after\n';
  text += '2. Never simulate, guess, or fabricate results\n';
  text += '3. Never add markdown, code blocks, or explanations around the tool call\n';
  text += '4. After receiving the result, you may respond with text\n';
  text += '5. Keep arguments compact — no large file contents\n\n';
  ...
  for (const tool of tools) {
    if (tool.type === 'function' && tool.function) {
      ...
      if (fn.parameters) {
        text += `  params: ${JSON.stringify(fn.parameters)}\n`;
      }
    }
  }
  text += '\n</tool_instructions>\n';
