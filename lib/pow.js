...
async function createPOW(headers, wasmUrl, targetPath = '/api/v0/chat/completion') {
  ...
  if (!cr.ok) throw new Error(`DeepSeek auth/network error while creating PoW challenge: HTTP ${cr.status}. Run npm run doctor. If auth expired, run npm run auth or npm run auth:import.`);
  let chalJson;
